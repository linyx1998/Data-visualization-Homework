...
with open("node2.json",'r') as load_f:
    load_dict = json.load(load_f)
    # print(load_dict)
print(np.array(load_dict).shape)

with open("link2.json",'r') as load_f:
    load_dict_link = json.load(load_f)
    # print(load_dict_link)
print(np.array(load_dict_link).shape)
...
for i in range(np.array(load_dict).shape[0]):
    ...
    elif load_dict[i].startswith("IP_C"):
        temp_dict["name"] = "IP_C"
...
with open("../../subgraph2.json","w") as f:
